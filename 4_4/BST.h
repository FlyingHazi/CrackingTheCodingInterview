	...
	public:
		...
		void levelTraversal()
		{
			...
			while(0 != ls.size())
			{
				lists.push_back(ls);
				ls.clear();
				for(typename list<node*>::iterator it=lists.back().begin();it!=lists.back().end();it++)
				{
					if(NULL != (*it)->leftChild)
					{
						ls.push_back((*it)->leftChild);
					}
					if(NULL != (*it)->rightChild)
					{
						ls.push_back((*it)->rightChild);
					}
				}
			}
			for(typename list<list<node*> >::iterator i=lists.begin();i!=lists.end();i++)
			{
				for(typename list<node*>::iterator j=i->begin();j!=i->end();j++)
				{
					cout<<(*j)->key<<'\t';
				}
				cout<<endl;
